...
function spendEnergy(game, amount) {
  if (game.energy < amount) return false;
  game.energy -= amount;
  game.lastEnergyGain = 0;
  return true;
}
...
export function canBuildTower(game, slotId) {
  return game.status!=="won" && game.status!=="lost"
    && MAP.towerSlots.some(s=>s.id===slotId)
    && !game.towers.some(t=>t.slotId===slotId)
    && game.towers.length<CONFIG.maxTowers
    && game.energy>=CONFIG.towerBuildCost;
}
...
export function canUpgradeTower(game,slotId){
  ...
  return Boolean(tower)
    && tower.level<CONFIG.maxTowerLevel
    && game.status!=="won" && game.status!=="lost"
    && game.energy>=CONFIG.towerUpgradeCost;
}

export function upgradeTower(game,slotId){
  if(!canUpgradeTower(game,slotId))return false;
  if(!spendEnergy(game,CONFIG.towerUpgradeCost))return false;
  const tower=game.towers.find(t=>t.slotId===slotId);
  tower.level+=1;
  tower.cooldown=0;
  return true;
...
export function moveHero(game,point){
  ...
  const d=clampPoint(point);
  game.hero.anchor={...d};
  game.hero.manualDestination={...d};
...
function moveEnemy(enemy,type,dt){
...
function nearestTarget(enemies,origin,range){
  return enemies.filter(e=>distance(e.position,origin)<=range).sort((a,b)=>b.progress-a.progress)[0];
}

function towerAttack(tower,enemies,origin,dt){
  const stats=TOWER_LEVELS[tower.level];
...
export function updateGame(game,dt){
  ...
  if(game.spawnQueue.length&&game.spawnTimer<=0){
    const n=game.spawnQueue.shift();
    spawnEnemy(game,n.type);
  ...
  for(const e of game.enemies){
    ...
    e.attackCooldown=Math.max(0,e.attackCooldown-step);
    e.engaged=false;
    e.engagement=null;
    ...
    if(type.combat==="ranged"){
      ...
      if(canShoot){
        e.engaged=true;
        e.engagement="ranged";
        if(e.attackCooldown<=0){game.hero.health-=type.damage;e.attackCooldown=type.attackCooldown;}
  ...
  for(const e of game.enemies){
    if(e.health<=0)addEnergy(game,ENEMY_TYPES[e.type].energyReward);
  }
