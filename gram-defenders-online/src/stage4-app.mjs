...
const bossHud=document.querySelector("#boss-hud"),bossFill=document.querySelector("#boss-fill"),bossState=document.querySelector("#boss-state");
...
function updateUi(){
  ...
  const elite=getElite(game);
  bossHud.hidden=!elite;
  if(elite){
    const type=ENEMY_TYPES[elite.type],ratio=Math.max(0,elite.health/elite.maxHealth);
