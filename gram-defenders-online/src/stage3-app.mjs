...
function updateUi(){
  if(game.status==="won"&&!victoryRecorded){completeStage(3,game.stars);lastStageReward=grantStageReward(3,game.stars).reward;victoryRecorded=true;const mapLink=document.querySelector(".stage-link");if(mapLink){mapLink.href=`/levels.html?completed=3&stars=${game.stars}`;mapLink.textContent="Continue · Stage 04";}}
  ...
  if(game.status==="won")message.textContent=`STAGE 03 COMPLETE · ${formatStars(game.stars)} · ${formatStageReward(lastStageReward)} · Stage 04 unlocked.`;
