...
function render(now){
  ...
  const selected=MAP.towerSlots.find(s=>s.id===selectedSlotId),selectedTower=game.towers.find(t=>t.slotId===selectedSlotId);if(selected&&selectedTower)drawTowerRange(selected,selectedTower.level,selectedTower.cardId);
...
function updateUi(){
  ...
  const cd=game.hero.skillCooldown;skillButton.disabled=game.status!=="playing"||Boolean(game.hero.manualDestination)||game.hero.downTimer>0||cd>0;skillButton.textContent=cd>0?`${game.battleMeta.hero.skillName} · ${cd.toFixed(1)}s`:game.battleMeta.hero.skillName;
...
const towerDeckUi=installTowerDeckUi(game,towerCard,{buildCost:cfg.buildCost,onSelect:(cardId)=>{game.selectedTowerCardId=cardId;setTowerMode("build");}});
...
document.querySelector("#restart").addEventListener("click",()=>{game=createGame(stageId);selectedSlotId=null;pulseFxUntil=0;lastShownEnergy=game.energy;victoryRecorded=false;lastStageReward=null;towerDeckUi.select(game.selectedTowerCardId);setTowerMode("build");last=performance.now();});
